...
#================================================================================
'''
 We will use a function from the Optimize_Functions.py script from an adptation of Daniel Portik's pipeline:
...
def IM_ae_b(params, ns):
    """
    nuae= ancestral pop size after expansion/contraction
	nu1= pop size for pop 1
	nu2=pop size for pop 2
	Tae= time of ancestral pop size change
	T1= time of split
...
for i in range(1,11):
    ...
    Optimize_Functions.Optimize_Routine(data, prefix, "IM_ae_b", IM_ae_b, 4, 8, data_folded=False, param_labels = p_labels, in_upper=upper, in_lower=lower, reps = reps, maxiters = maxiters, folds = folds)
